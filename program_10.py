...
# 
# Modified by Shizhang Wang
# 4/20/2020
# the script below takes two USGS streamflow data file, clean up (remove NaN) and
# check for gross error (<0), then calculate a variety of statistics and environmental
# metrics after grouping by water year or months then output for report files 
# of (average) annual and month metrics
...
def CalcTqmean(Qvalues):
    ...
    # since NaN is removed in the first step, mean was calculated directly
    Qvalues=Qvalues.dropna() # just be cautious
    # Qmean = Qvalues.mean()
...
def Calc7Q(Qvalues):
    ...
    Qvalues = Qvalues.dropna() # remove NaN
    # calulate a 7 day averages then find the min value
...
def GetAnnualStatistics(DataDF):
    ...
    starts on October 1."""
    # resample and aggregate into annual values
    annualData = DataDF.resample('AS-OCT')
    ...
    # populate with predefined functions
    ...
    WYDataDF['R-B Index'] = annualData.apply({'Discharge': lambda x: CalcRBindex(x)})
    WYDataDF['7Q'] = annualData.apply({'Discharge': lambda x: Calc7Q(x)})
    WYDataDF['3xMedian'] = annualData.apply({'Discharge': lambda x: CalcExceed3TimesMedian(x)})
...
def GetMonthlyStatistics(DataDF):
    ...
    # populate new DF with stats
    MoDataDF['site_no'] = monthlyData['site_no'].mean()
    MoDataDF['Mean Flow'] = monthlyData['Discharge'].mean()
    MoDataDF['Coeff Var'] = ((monthlyData['Discharge'].std())/monthlyData['Discharge'].mean())*100
    MoDataDF['Tqmean'] = monthlyData.apply({'Discharge': lambda x: CalcTqmean(x)})
    MoDataDF['R-B Index'] = monthlyData.apply({'Discharge': lambda x: CalcRBindex(x)})
...
def GetAnnualAverages(WYDataDF):
    ...
    AnnualAverages=WYDataDF.mean() # default column wise 
...
def GetMonthlyAverages(MoDataDF):
    ...
    # groupby individual month
    MonthlyAverages=MoDataDF.groupby(MoDataDF.index.month).mean() 
    # from coursera, https://www.coursera.org/learn/python-data-analysis/lecture/31dEi/date-functionality
...
if __name__ == '__main__':
    ...
    WY_tipp = WYDataDF['Tippe']
    WY_tipp['Station']='Tippe'
    WY_all = WY_wild.append(WY_tipp)
    WY_all.to_csv('Annual_Metrics.csv', sep=',',index=True)
    
    # monthly metrics
    Mo_wild = MoDataDF['Wildcat']
    Mo_wild['Station']='Wildcat'
    Mo_tipp = MoDataDF['Tippe']
    Mo_tipp['Station']='Tippe'
    Mo_all = Mo_wild.append(Mo_tipp)
    Mo_all.to_csv('Monthly_Metrics.csv', sep=',',index=True)
    
    # annual average
    WYA_wild = AnnualAverages['Wildcat']
    WYA_wild['Station']='Wildcat'
    WYA_tipp = AnnualAverages['Tippe']
    WYA_tipp['Station']='Tippe'
    WYA_all = WYA_wild.append(WYA_tipp)
    WYA_all.to_csv('Average_Annual_Metrics.txt', sep='\t',index=True)
    
    # monthly average
    MoA_wild = MonthlyAverages['Wildcat']
    MoA_wild['Station']='Wildcat'
    MoA_tipp = MonthlyAverages['Tippe']
    MoA_tipp['Station']='Tippe'
    MoA_all = MoA_wild.append(MoA_tipp)
    MoA_all.to_csv('Average_Monthly_Metrics.txt', sep='\t',index=True)
